...
const tagPosts = async tags => {
  ...
  SELECT
    posts.id as postsId, co.images, co.company_name, posts.title, ps.tech_stacks, co.location, posts.career_min, career_max, view, posts.position_id, posts.company_id,
    posts.due_date
  FROM posts
    LEFT JOIN (
        SELECT
...
WHERE career_min < ${career} < career_max
`);
